...
if( !class_exists( 'JLCEpointPersonalTrainerEditFoodCategoryForm' ) )
{

class JLCEpointPersonalTrainerEditFoodCategoryForm extends JLCCustomForm
{
	public function __construct( $internal_id, $args )
	{
		$text_domain = isset( $args['text_domain'] ) ? $args['text_domain'] : null;
		$category_id = isset( $args['category'] ) ? $args['category'] : null;
		$category = $category_id ? EpointPersonalTrainerMapper::get_food_category( $category_id ) : null;

		parent::__construct(
			__DIR__,
			$internal_id,
			$text_domain,
			'epoint_personal_trainer_edit_food_category',
			true,//ajax
	...
	protected function process_form()
	{
		...
		if( empty( $cat_id ) || !( $category = EpointPersonalTrainerMapper::get_food_category( $cat_id ) ) )
			return array(
		...
		$res = EpointPersonalTrainerMapper::update_food_category( $cat_id, $name, 1, true );

		if( $res !== false )
		{
			$this->store_response( 'Categoría &quot;' . $name . '&quot; actualizada correctamente.' );
