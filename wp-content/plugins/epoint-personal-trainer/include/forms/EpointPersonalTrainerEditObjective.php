...
if( !class_exists( 'JLCEpointPersonalTrainerEditObjectiveForm' ) )
{

class JLCEpointPersonalTrainerEditObjectiveForm extends JLCCustomForm
{
	...
	public function __construct( $internal_id, $args )
	{
		...
		$objective = $objective_id ? EpointPersonalTrainerMapper::get_objective( $objective_id ) : null;
		...
		parent::__construct(
			...
			$internal_id,
			$text_domain,
			'epoint_personal_trainer_edit_objective',
			false,//true,//ajax
		...
		$this->class = 'edit-objective-form';
...
*/
	...
	protected function process_form()
	{
		...
		if( !$objective_id )
		{
			$objective_id = EpointPersonalTrainerMapper::create_objective(
				$name,
				$position,
				$active ? 1 : 0,
				get_current_user_id()
		...
		else
		{
			...
			$updated = EpointPersonalTrainerMapper::update_objective(
				$objective_id,
				$name,
				$position,
				$active ? 1 : 0
