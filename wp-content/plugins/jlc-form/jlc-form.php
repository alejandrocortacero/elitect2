 ...
 * Version: 1.4.68
 * Author: JLC
 * AUthor URI: www.jlcdevelopment.es
 * Text Domain: jlc-custom-form-textdomain
 * License: GNU
 */

//TODO: use locate template in look_for_field( $filename ) methods
//TODO: MORE IMPORTANT que se incluya ajax.js cuando hay un campo que lo requiera en enqueue_global_ajax_script, habrá que añadir un método similar a has_ajax_callable
//TODO: IMPORTANT jlc_custom_form_user cookie: add_sameSite and add more entropy
...
// Version 1.4.22 - get_current_url() modified: A conditional added for REDIRECT_URL
// Version 1.4.23 - set_value( $values ) added to CheckboxGroup
//					yes_no_group accept value setting on creation
// Version 1.4.24 - time field added
// Version 1.4.25 - jlc-calendar field added
// Version 1.4.26 - hidden input inside upload-ajax-image field triggers 'change' when is changed
//					upload-ajax-image field download image after refresh if value is set (check_image_exists action)
// Version 1.4.27 - storable property added
// Version 1.4.28 - REDIRECT_URL removed from get_current_url()
// Version 1.4.29 - post_select field has been fixed. Now "allow_new_options" is set to false in parent constructor
// Version 1.4.30 - admin-settings-form replace \" with " for wp_editor and tinymce fields
// Version 1.4.31 - jlc-calendar preselected values management updated and require validation added
// Version 1.4.32 - unsetImage() method added for ajax_upload_image field
// Version 1.4.33 - manual form registration added (aun no)
// Version 1.4.34 - radio_button_group in field loader fixed
// Version 1.4.35 - user_select field added
// Version 1.4.36 - upload_ajax_image_cropper field added
// Version 1.4.37 - JLCPartialForm class added and its support:
//					register_partial_form()
//					unregister_partial_form()
//					initialize_partial_forms()
//					
//					JLCPostMetaBoxForm added
//					load_abstract_file( $file ) method added
// Version 1.4.38 - upload-ajax.js: checkValueAtRefresh updated (also in field templates)
// Version 1.4.39 - partial metabox form only saves at action = editpost
// Version 1.4.40 - partial metabox form style added (method print_fields_style)
//					get_default_template_path() added
// Version 1.4.41 - partial metabox form preload values from transient
//					upload_ajax_image_cropper field works responsive with proportions
// Version 1.4.42 - stars control added
// Version 1.4.43 - fields_array has new method: add_field_object()
// Version 1.4.44 - spanish_province_select_field fixed
// Version 1.4.45 - JLCCustomFormUploadAjaxImageField checkImageExists action: added sprintf to fill with field values (upload-ajax.js also replace string values)
// Version 1.4.46 - textarea admin field "rows" and "wrap" properties fixed
//					protected function enqueue_custom_public_scripts() {} added
//					protected function enqueue_custom_admin_scripts() {} added
// Version 1.4.47 - table_field added
//					labels tag are not printed if empty in field admin templates
//					input css for metabox is modified (buttons in wp_editor were affected before)
//					get_tmp_file() added in upload_file field
//					term select label fixed
//					initialize_partial_forms and initialize_forms_reading has priority 99
//					optgroup support added to select field
//					option to enqueue all admin styles added
// Version 1.4.48 - ajax forms trigger 'jlccustomformAjaxSuccessEnd' event
//					upload_ajax_image_cropper refreshes at 'jlccustomformAjaxSuccessEnd' event
// Version 1.4.49 - country select added
//					read_request() in spanish_province_select fixed
// Version 1.4.50 - table variable field added
//					select field accepts and parse options in constructor as array
// Version 1.4.51 - method set_value() added to array field.
// Version 1.4.52 - new events triggered at submitting ajax form (beforeSend y completed)
// Version 1.4.53 - table_variable improved
//					term_fields_form partial form added
// Version 1.4.54 - upload private image field added
// Version 1.4.55 - upload private image field improved
// Version 1.4.56 - none option label in post_select field is configurable
// Version 1.4.57 - look_for_field form method also looks in specific form folder
//					look_for_field abstract field method also looks in specific form folder
//					get_registered_form_dir() method added
// Version 1.4.58 - some public field templates has class form-group-{name} in form-group layer
// Version 1.4.59 - registered-forms-table does not fail when file is not found
// Version 1.4.60 - stars field: triggers change event
// Version 1.4.61 - style changed for admin upload-ajax-image-field
// Version 1.4.62 - look_for_field form and abstract field methods removes ABSPATH from $form_dir if is contained
// Version 1.4.63 - upload-ajax-image-croppers updated to work in post-load genereted forms
// Version 1.4.64 - table-variable field works on public, admin and post-meta-box
// Version 1.4.65 - table-variable field allows nested table-variable-field
// Version 1.4.66 - generate_wpajax_response( $responses ) returns message wrapper depending on HTTP_REFERER (admin or not)
//					ajax.js also removes .notice elements before form submitting
//					jquery-slider admin template added
// Version 1.4.67 - readFields on ajax.js also submits submitter
// Version 1.4.68 - ajax scripts are loaded if form includes an ajax_upload_image field (see has_ajax_fields())
...
abstract class JLCCustomForm
{
	...
	const REGISTERED_PARTIAL_FORMS_KEY	= 'jlc_custom_forms_registered_partial_forms';
	...
	public static function load_abstract_file( $file )
	{
		require_once( implode( DIRECTORY_SEPARATOR, array( __DIR__, 'abstract', $file ) ) );
	}
	...
	public static function register_partial_form( $internal_id, $dir, $file, $args, $from )
	{
		$registered_forms = get_option( self::REGISTERED_PARTIAL_FORMS_KEY, array() );
		...
		if( !$already_registered )
		{
			...
			update_option( self::REGISTERED_PARTIAL_FORMS_KEY, $registered_forms );
		}
	}
	...
	public static function unregister_partial_form( $internal_id, $dir, $file )
	{
		$registered_forms = get_option( self::REGISTERED_PARTIAL_FORMS_KEY, array() );
		$new_forms = array();
		...
		update_option( self::REGISTERED_PARTIAL_FORMS_KEY, $new_forms );
	}

	public static function get_registered_form_dir( $internal_id )
	{
		$registered_forms = get_option( self::REGISTERED_FORMS_KEY, array() );
	...
	/**
	 * Partial forms do not comply with the principle of
	 * selective loading:
	 *
	 * They have to hook Wordpress actions and filters always,
	 * while other forms are only registered when they have
	 * been submitted.
	 *
	 * Partial forms require reading initialization,
	 * like other forms, but also printing initialization.
	 *
	 */
	public static function initialize_partial_forms()
	{
		// initializes user cookie if not exists
		self::get_user_cookie();
		...
		{
			$matches = array();
			if( is_dir( ABSPATH . $form['dir'] ) &&
				is_readable( ABSPATH . $form['dir'] . DIRECTORY_SEPARATOR . $form['file'] ) &&
				preg_match( '/^(.*)\.php$/', $form['file'], $matches )
			) {
				$form = self::get_form( $form['internal_id'], $form['dir'], $matches[1], $form['args'] );
				if( method_exists( $form, 'register_hooks' ) )
					$form->register_hooks();
			}
		}
	}
	...
	public static function get_form( $internal_id, $dir, $name, $args = null )
	{
		...
		if( $args !== null )
			$new_form = new $class_name( $internal_id, $args );
		else
			$new_form = new $class_name( $internal_id );

		if( !$new_form->is_storable() )
			return $new_form;

		self::$loaded_forms[$internal_id] = $new_form;
	...
	{
		if( get_option( 'jlc_custom_forms_admin_enqueue_all_styles' ) === 'yes' )
		{
			wp_enqueue_style(
				'jlc-custom-form-tabbed-admin-form-style',
				plugins_url( 'templates/admin/css/tabbed-form.css', __FILE__ ),
				array(),
			...
			wp_enqueue_style(
				...
				plugins_url( '/templates/css/search-form.css', __FILE__ ),
				array(),
				self::VERSION
			);
			...
			wp_enqueue_style(
				...
				plugins_url( 'fields/templates/css/jlc-calendar.css', __FILE__ ),
				array(),
				JLCCustomForm::VERSION
			);
			...
			wp_enqueue_style(
				...
				plugins_url( 'fields/templates/css/stars.css', __FILE__ ),
				array(),
				JLCCustomForm::VERSION
			);
			...
			wp_enqueue_style(
				...
				plugins_url( 'fields/templates/css/table-admin.css', __FILE__ ),
				array(),
				JLCCustomForm::VERSION
			);
			...
			wp_enqueue_style(
				...
				plugins_url( 'fields/templates/css/table-variable-admin.css', __FILE__ ),
				array(),
				JLCCustomForm::VERSION
			);
		}
	...
	public static function print_admin_registered_forms_page()
	{
		if( isset( $_GET['help'] ) )
		{
			$file = implode( DIRECTORY_SEPARATOR, array( __DIR__, 'help.php' ) );
		...
		{
			$file = implode( DIRECTORY_SEPARATOR, array( __DIR__, 'templates', 'admin', 'registered-forms.php' ) );
...
/*
		...
		{
//OLD_TODO: RECHECK THIS URGENT he comentado la siguiente linea y añadido una copia sin HTTP_HOST ni lo precedente
...
//searched $_SERVER["REDIRECT_URL"] for a while and noted that it is not mentioned in php documentation page itself. look like this is only generated by apache server(not others) and using   $_SERVER["REQUEST_URI"] will be useful in some cases as mine.
			...
			$url = preg_match( '/^http/', $_SERVER['REDIRECT_URL'] ) ?
				$_SERVER['REDIRECT_URL']
				:
				( isset( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] === 'on' ? "https" : "http" ) . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REDIRECT_URL']
			;
...
*/
	...
	public function add_class( $html_class )
	{
	...
	protected function initialize_data_fields_reading()
	{
		foreach( $this->get_data_fields() as $field )
		{
			...
			{
				add_action( 'wp_ajax_' . sprintf( $field::AJAX_CHECK_IMAGE_EXISTS_ACTION, $this->internal_id, $field->get_name() ), array( $field, 'check_image_exists' ) );
				if( !$this->private )
					add_action( 'wp_ajax_nopriv_' . sprintf( $field::AJAX_CHECK_IMAGE_EXISTS_ACTION, $this->internal_id, $field->get_name() ), array( $field, 'check_image_exists' ) );
	...
	protected function add_stars_field( $name, $args = array() )
	{
		return $this->add_field( JLCCustomFormFieldLoader::get_stars_field( $name, $args ) );
	}
	...
	protected function add_time_field( $name, $args = array() )
	{
		return $this->add_field( JLCCustomFormFieldLoader::get_time_field( $name, $args ) );
	}

	protected function add_calendar_field( $name, $args = array() )
	{
		return $this->add_field( JLCCustomFormFieldLoader::get_calendar_field( $name, $args ) );
	}
	...
	protected function add_user_select( $name, $args = array() )
	{
		return $this->add_field( JLCCustomFormFieldLoader::get_user_select( $name, $args ));
	}
	...
	protected function add_ajax_upload_image_cropper_field( $name, $args = array() )
	{
		return $this->add_field( JLCCustomFormFieldLoader::get_ajax_upload_image_cropper_field( $name, $args ) );
	}
	...
	protected function add_upload_private_image_field( $name, $args )
	{
		if( $this->enctype === null )
			$this->enctype = "multipart/form-data";

		return $this->add_field( JLCCustomFormFieldLoader::get_upload_private_image_field( $name, $args ) );
		
	}

	protected function add_audio_recorder_field( $name, $args )
	{
		return $this->add_field( JLCCustomFormFieldLoader::get_audio_recorder_field( $name, $args ) );
	}
/*
...
*/
	...
	protected function add_country_select( $name, $args = array() )
	{
		return $this->add_field( JLCCustomFormFieldLoader::get_country_select( $name, $args ) );
	}
	...
	protected function add_spanish_city_select( $name, $args = array() )
	{
		return $this->add_field( JLCCustomFormFieldLoader::get_spanish_city_select( $name, $args ) );
	}
	...
	protected function add_table_field( $name, $args = array() )
	{
		return $this->add_field( JLCCustomFormFieldLoader::get_table_field( $name, $args ) );
	}
	protected function add_table_variable_field( $name, $args = array() )
	{
		return $this->add_field( JLCCustomFormFieldLoader::get_table_variable_field( $name, $args ) );
	}
	...
	protected static function get_default_template_path( $filename )
	{
	...
	protected function look_for_file( $filename )
	{
		...
		if( mb_strpos( $base_dir, ABSPATH ) === 0 )
			$base_dir = mb_substr( $base_dir, mb_strlen( ABSPATH ) );
		...
		return self::get_default_template_path( $filename );
	}
...
*/
	...
	{
		if( $this->has_ajax_fields() || $this->has_ajax_callable_fields() || ( $this->ajax && $this->wordpress_method ) )
		{
			ob_start();
	...
	protected function enqueue_custom_admin_scripts() {}
	...
	protected function has_ajax_fields()
	{
		foreach( $this->get_data_fields() as $field )
			if( is_a( $field, 'JLCCustomFormUploadAjaxImageField' ) ) // TODO: Improve this: add something to fields that require ajax.js
				return true;
	...
	public function print_public_form( $hide_messages = false )
	{
		...
		$this->enqueue_custom_public_scripts();
	}
	...
	public function print_admin_form( $readonly_form = false )
	{
		...
		$this->enqueue_custom_admin_scripts();
	}
...
// TODO: procesar $_FILES en AJAX
	 ...
	 * It always ends creating a redirection and setting
	...
	protected function store_response( $result )
	{
		$transient_name = $this->get_messages_transient_name();
		if( $transient_name )
		{
	...
	protected function generate_wpajax_response( $responses )
	{
		...
		{
			ob_start();
			if( !empty( $_SERVER['HTTP_REFERER'] ) && mb_strpos( $_SERVER['HTTP_REFERER'], get_admin_url() ) === 0 )
			{
				foreach( $responses as $r )
				{
					$message = $r['text'];
					if( !$r['code'] )
						include( implode( DIRECTORY_SEPARATOR, array( __DIR__, 'templates', 'admin', 'success_notice.php' ) ) );
					else
						include( implode( DIRECTORY_SEPARATOR, array( __DIR__, 'templates', 'admin', 'error_notice.php' ) ) );
				}
			}
			else
			{
	...
	{
		if( !$this->private && !is_user_logged_in() )
			return true;
...
if( !has_action( 'init', array( 'JLCCustomForm', 'initialize_forms_reading' ) ) )
	add_action( 'init', array( 'JLCCustomForm', 'initialize_forms_reading' ), 99, 0 );

if( !has_action( 'init', array( 'JLCCustomForm', 'initialize_partial_forms' ) ) )
	add_action( 'init', array( 'JLCCustomForm', 'initialize_partial_forms' ), 99, 0 );

if( !has_action( 'admin_enqueue_scripts', array( 'JLCCustomForm', 'admin_enqueue_scripts' ) ) )
	add_action( 'admin_enqueue_scripts', array( 'JLCCustomForm', 'admin_enqueue_scripts' ) );
