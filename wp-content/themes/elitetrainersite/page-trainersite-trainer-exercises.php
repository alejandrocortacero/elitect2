...
add_filter( 'body_class', function( $classes, $class ) { $classes[] = 'page-trainer-exercises-list'; return $classes; }, 10, 2 );
...
$last_duplicated_exercise = isset( $_COOKIE[EliteTrainerSiteTheme::EXERCISE_LAST_DUPLICATED_COOKIE] ) ? (int)$_COOKIE[EliteTrainerSiteTheme::EXERCISE_LAST_DUPLICATED_COOKIE] : null;
if( $last_duplicated_exercise )
{
	setCookie( EliteTrainerSiteTheme::EXERCISE_LAST_DUPLICATED_COOKIE, '', time() + 24*60*60*1000, '/' );
}

if( !empty( $_COOKIE['exercises_gallery_message'] ) )
{ 
	$exercises_gallery_message = $_COOKIE['exercises_gallery_message'];
	setCookie( 'exercises_gallery_message', '', time() + 24*60*60*1000, '/' );
}
			...
			<div class="col-xxs-12 col-xs-12">
				<div class="row">
					<div class="col-xs-12 page-content">
						<h1 style="display:flex;justify-content:space-between;align-items:flex-start;">Ejercicios / Categorías <a class="btn btn-primary pull-right" href="<?php echo EliteTrainerSiteTheme::get_creation_zone_url(); ?>" rel="nofollow">Volver al panel principal</a></h1>
						<?php if( !empty( $_COOKIE['exercises_gallery_message'] ) ) : ?>
							<div class="alert alert-success"><p><?php echo wp_kses_post( $exercises_gallery_message ); ?></p></div>
						<?php endif; ?>
						<div class="tools-bar">
							<div class="exercise-tools" style="display:flex;width:100%;">
								<a class="btn btn-primary" style="width:100%;" href="<?php echo EliteTrainerSiteTheme::get_edit_exercise_url( null ); ?>" rel="nofollow">Crear nuevo ejercicio</a>
								<a class="btn btn-primary" style="width:100%;" href="#" rel="nofollow" data-toggle="modal" data-target="#add-exercise-category-modal">Crear nueva categoría</a>
								<a class="btn btn-primary" style="width:100%;" href="#" rel="nofollow" data-toggle="modal" data-target="#exercise-categories-modal">Editar mis categorías</a>
							</div>
								...
								<?php $exercises = EpointPersonalTrainerMapper::get_trainer_exercises( $user->ID ); ?>
									...
									<div class="table-responsive"><table class="table table-striped items-table">
										...
										<?php foreach( $exercises as $exercise ) : ?>
											...
											<tr class="exercise <?php if( $last_duplicated_exercise == $exercise->ID ) : ?>new-duplicated<?php endif; ?> <?php foreach( $category_ids as $cat_id ) : ?> exercise-category-<?php echo esc_attr( $cat_id ); ?><?php endforeach; ?>">
...
<?php if( EliteTrainerSiteTheme::must_show_duplicate_exercise_confirmation() ) : ?>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
			<p>Este ejercicio de duplicará y aparecerá en tu listado de ejercicios propios, dándote la opción de  modificar las fotos o texto o ambos y así tener un nuevo ejercicio en tu listado de "Mis Ejercicios" de forma cómoda y rápida.</p>
...
<?php if( EliteTrainerSiteTheme::must_show_delete_exercise_confirmation() ) : ?>
...
<?php if( EliteTrainerSiteTheme::must_show_edit_exercise_confirmation() ) : ?>
...
<div class="modal fade" id="add-exercise-category-modal" tabindex="-1" role="dialog" aria-labelledby="add-exercise-category-modal-label">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="add-exercise-category-modal-label">Crear nueva categoría</h4>
      </div>
      <div class="modal-body">
			<?php echo EpointPersonalTrainerPublic::get_add_exercise_category_form(); ?>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="exercise-categories-modal" tabindex="-1" role="dialog" aria-labelledby="exercise-categories-modal-label">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="exercise-categories-modal-label">Categorías de ejercicio</h4>
      </div>
      <div class="modal-body">
		<?php if( false )  : ?><a class="btn btn-primary" href="#" rel="nofollow" onclick="jQuery('#exercise-categories-modal').modal('hide');" data-toggle="modal" data-target="#add-exercise-category-modal">Crear nueva categoría</a><?php endif; ?>
		<?php $categories = EpointPersonalTrainerMapper::get_trainer_exercise_categories( get_current_user_id(), null, null, 'name', 'ASC' ); ?>
			...
			<table class="table table-striped items-table">
				...
				<?php foreach( $categories as $cat ) : ?>
					...
					<tr class="cat-cel edit-cat-cel" style="display:none;">
						<td colspan="2">
							<?php echo EpointPersonalTrainerPublic::get_edit_exercise_category_form( $cat->ID ); ?>
						</td>
					</tr>
