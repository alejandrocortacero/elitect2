...
abstract class JLCPageHeaderVideoAbstractForm extends JLCCustomForm
{
	...
	public function __construct(
		...
		parent::__construct(
			...
			$internal_id,
			$text_domain,
			'elite_trainer_site_' . $video_name . '_video',
			true,//ajax
		...
		$this->video_selector = '.page-header-video-container .title-row .video-layer .video';
		...
		$this->add_checkbox_field(
			'pageheadervideoenabled_' . $video_name,
			array(
				'value' => 'yes',
				'checked' => EliteTrainerSiteThemeCustomizer::is_page_header_video_enabled( $video_name ),
	...
	protected function process_form()
	{
		...
		if( $restore_val )
		{
			...
			delete_blog_option( null, EliteTrainerSiteThemeCustomizer::PAGE_HEADER_VIDEO_ENABLED_PREFIX . $this->video_name );
		}
		else
		{
			...
			$enabled = $this->get_field_by_name( 'pageheadervideoenabled_' . $this->video_name )->is_checked();
			EliteTrainerSiteThemeCustomizer::set_page_header_video_enabled( $this->video_name, $enabled );
		}

		$is_video = !empty( EliteTrainerSiteThemeCustomizer::get_video_link( $this->video_name ) ) && EliteTrainerSiteThemeCustomizer::is_page_header_video_enabled( $this->video_name );
				...
				'data' => json_encode( array(
					...
					'args' => array(
						...
						'fields' => array(
							...
							array(
								'fieldName' => 'pageheadervideoenabled_' . $this->video_name,
								'fieldType' => 'checkbox',
								'fieldValue' => false 
							)
						)
