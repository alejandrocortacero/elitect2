...
if( !class_exists( 'JLCSubMenu2Form' ) )
{

class JLCSubMenu2Form extends JLCCustomForm
{
	public function __construct( $internal_id, $args )
	{
		...
		$color_field = $this->add_color_field(
			'color',
			array(
				'value' => EliteTrainerSiteThemeCustomizer::get_submenu2_text_color(),
				'label' => __( 'Color de texto', $this->get_text_domain() ),
				'required' => true
			)
		);
		$color_field->set_datalist( EliteTrainerSiteThemeCustomizer::get_option_history( EliteTrainerSiteThemeCustomizer::SUBMENU2_TEXT_COLOR_KEY ) );

		$bg_color_field = $this->add_background_field(
			'bgcolor',
			array(
				'value' => EliteTrainerSiteThemeCustomizer::get_submenu2_bg(),
		...
		$font_size_field = $this->add_number_field(
			'fontsize',
			array(
				'value' => EliteTrainerSiteThemeCustomizer::get_submenu2_font_size(),
		...
		$font_size_field->set_datalist( EliteTrainerSiteThemeCustomizer::get_option_history( EliteTrainerSiteThemeCustomizer::SUBMENU2_FONT_SIZE_KEY ) );

		$font_family = EliteTrainerSiteThemeCustomizer::get_submenu2_font_family();
...
*/
	protected function process_form()
	{
		...
		EliteTrainerSiteThemeCustomizer::set_submenu2_text_color( $color );

		$bg = $this->get_field_by_name( 'bgcolor' )->get_value();
		EliteTrainerSiteThemeCustomizer::set_submenu2_bg( $bg );
		
		$fontsize = $this->get_field_by_name( 'fontsize' )->get_value();
		EliteTrainerSiteThemeCustomizer::set_submenu2_font_size( $fontsize );

		$fontfamily = $this->get_field_by_name( 'fontfamily' )->get_value();
		EliteTrainerSiteThemeCustomizer::set_submenu2_font_family( $fontfamily );
