...
/*
		...
		$this->add_html( array( 'content' => '<div class="current-background-image"><img src="' . EliteTrainerSiteTheme::get_home_cover_background_url() . '" alt="Background" /></div>' ) );
		...
		$this->add_text_field(
			'cover_subtitle',
			array(
				'value' => get_blog_option( get_current_blog_id(), EliteTrainerSiteTheme::HOME_SUBTITLE_KEY ),
		...
		$home = get_post( get_blog_option( get_current_blog_id(), EliteTrainerSiteTheme::HOME_PAGE_KEY ) );
...
/*
		...
		if( $cover_background_field->is_file_for_upload() && $cover_background_field->is_upload_ok() )
		{
			...
			update_blog_option( get_current_blog_id(), EliteTrainerSiteTheme::COVER_BACKGROUND_KEY, $logo_id );
		}

		$cover_subtitle = $this->get_field_by_name( 'cover_subtitle' )->get_value();
		update_blog_option( get_current_blog_id(), EliteTrainerSiteTheme::HOME_SUBTITLE_KEY, $cover_subtitle );

		$home_content = $this->get_field_by_name( 'home_content' )->get_value();
		$home_id = get_blog_option( get_current_blog_id(), EliteTrainerSiteTheme::HOME_PAGE_KEY );
