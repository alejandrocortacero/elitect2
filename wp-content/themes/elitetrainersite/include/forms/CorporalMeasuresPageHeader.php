...
if( !class_exists( 'JLCCorporalMeasuresPageHeaderForm' ) )
{

if( !class_exists( 'JLCHtmlAbstractForm' ) )
	require_once( implode( DIRECTORY_SEPARATOR, array( __DIR__, 'HtmlAbstract.php' ) ) );

class JLCCorporalMeasuresPageHeaderForm extends JLCHtmlAbstractForm
{
	public function __construct( $internal_id, $args )
	{
		parent::__construct(
			'corporalmeasuresheadertext',
			$internal_id,
			$args,
			null,//form_title
		...
		$this->text_selector = '.corporal-measures-row.row .col-xs-12 .section-text';
	}
}
