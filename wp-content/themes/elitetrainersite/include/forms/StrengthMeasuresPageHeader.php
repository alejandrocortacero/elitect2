...
if( !class_exists( 'JLCStrengthMeasuresPageHeaderForm' ) )
{

if( !class_exists( 'JLCHtmlAbstractForm' ) )
	require_once( implode( DIRECTORY_SEPARATOR, array( __DIR__, 'HtmlAbstract.php' ) ) );

class JLCStrengthMeasuresPageHeaderForm extends JLCHtmlAbstractForm
{
	public function __construct( $internal_id, $args )
	{
		parent::__construct(
			'strengthmeasuresheadertext',
			$internal_id,
			$args,
			null,//form_title
		...
		$this->text_selector = '.strength-measures-row.row .col-xs-12 .section-text';
	}
}
