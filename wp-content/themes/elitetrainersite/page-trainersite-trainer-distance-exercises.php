...
<div class="container">
	<?php if( have_posts() ) : the_post(); ?>
		<div class="row">
			<div class="col-xxs-12 col-xs-12">
				<div class="row">
					...
					<?php if( !$magnitude_id ) : ?>
						<h1><a class="btn btn-primary pull-right" href="<?php echo EliteTrainerSiteTheme::get_creation_zone_url(); ?>" rel="nofollow">Volver al panel principal</a> Medidas de distancia</h1>
						<?php if( class_exists( 'EpointPersonalTrainer', false ) ) : ?>
							<div class="body-measures-layer">
							<?php $magnitudes = EpointPersonalTrainerMapper::get_evolution_magnitudes_by_type( 'distance', $user->ID ); ?>

								<p><a class="btn btn-primary" href="<?php echo EliteTrainerSiteTheme::get_trainer_distance_exercises_url( 'new' ); ?>" rel="nofollow">Nueva medida</a></p>
								<div class="table-responsive"><table class="table table-striped items-table">
									...
									<?php foreach( $magnitudes as $m ) : ?>
										<tr>
		<td><a href="<?php echo EliteTrainerSiteTheme::get_trainer_distance_exercises_url( $m->ID ); ?>"><?php echo $m->name; ?></a></td>
		...
		<?php endif; ?>
					...
					<?php else : ?>
						<h1>
							<a class="btn btn-primary pull-right" href="<?php echo EliteTrainerSiteTheme::get_trainer_distance_exercises_url(); ?>" rel="nofollow">Volver al listado</a>
							<?php if( $magnitude_id == 'new' ) : ?>Nueva medida<?php else: ?>Editar medida<?php endif; ?>
						</h1>
						<?php if( class_exists( 'EpointPersonalTrainer', false ) ) : ?>
							<?php echo EpointPersonalTrainerPublic::get_edit_evolution_magnitude_form( $magnitude_id != 'new' ? (int)$magnitude_id : null, 'distance' ); ?>
						<?php endif; ?>
					<?php endif; ?>
					</div>
				</div>
