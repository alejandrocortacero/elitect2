...
class EliteTrainerSiteThemeWooCommerce
{
	const ENABLE_FIXED_CART_KEY = 'elitetrainersite_settings_woo_fixed_cart';
	...
	public static function initialize()
	{
		...
		if( is_plugin_active( 'perfect-woocommerce-brands/main.php' ) )
		{
			...
			//add_image_size( 'elitetrainersite-brand', 400, 0, false );
			//add_image_size( 'elitetrainersite-small-brand', 100, 0, false );
		}
...
*/
		...
		if( self::is_fixed_cart_enabled() )
		{
			...
			add_action( 'elitetrainersitetheme_footer_after_static_content', array( get_class(), 'print_fixed_cart' ) );
		}

		add_filter( 'elitetrainersitetheme_default_menu_items', array( get_class(), 'default_menu_items' ) );
		add_filter( 'elitetrainersitetheme_legal_menu_items', array( get_class(), 'legal_menu_items' ) );
	}
	...
	public static function settings_fields( $fields )
	{
		$fields[] = array(
			...
			'args' => array(
				'value' => 'yes',
				'label' => __( 'Enable WooCommerce fixed cart', EliteTrainerSiteTheme::TEXT_DOMAIN ),
	...
	public static function default_menu_items( $items )
	{
		...
		if( $shop_page_link )
		{
			$items['shop'] = array( 'url' => $shop_page_link , 'text' => !empty( $shop_title ) ? $shop_title : __( 'Products', EliteTrainerSiteTheme::TEXT_DOMAIN ) );
		}

		$items['account'] = array( 'url' => self::get_my_account_url(), 'text' => __( 'My Account', EliteTrainerSiteTheme::TEXT_DOMAIN ), 'icon' => 'user' );
		//$ret['cart'] = array( 'url' => self::get_cart_link(), 'text' => __( 'My Cart', EliteTrainerSiteTheme::TEXT_DOMAIN ) );
