...
class EliteTrainerSiteThemeFacebook
{
	const FACEBOOK_APP_ID_KEY = 'elitetrainersite_facebook_app_id';
	const ENABLE_LIKE_BUTTON_SUPPORT_KEY = 'elitetrainersite_facebook_enable_like_button';
	...
	public static function initialize()
	{
		...
		add_action( 'elitetrainersitetheme_footer_after_static_content', array( get_class(), 'print_footer_tags' ), 90, 0 );
	}
	...
	public static function settings_fields( $fields )
	{
		$fields[] = array(
			...
			'args' => array(
				...
				'label' => __( 'Facebook app ID', EliteTrainerSiteTheme::TEXT_DOMAIN ),
				'required' => false
			)
		);
		...
		$fields[] = array(
			...
			'args' => array(
				'value' => 'yes',
				'label' => __( 'Enable Facebook like button support', EliteTrainerSiteTheme::TEXT_DOMAIN ),
	...
	public static function get_og_description()
	{
		...
		elseif( is_home() )
		{
			return sprintf( __( 'Read the latest articles from %s', EliteTrainerSiteTheme::TEXT_DOMAIN ), get_bloginfo( 'name' ) );
		}
		elseif( is_singular() )
		{
			if( class_exists( 'JLCPageMeta' ) )
	...
	public static function get_og_title()
	{
		...
		// If it's a search, use a dynamic search results title.
			...
			$title['title'] = get_bloginfo( 'name', 'display' ) . ' - ' . __( 'Catalog', EliteTrainerSiteTheme::TEXT_DOMAIN );
